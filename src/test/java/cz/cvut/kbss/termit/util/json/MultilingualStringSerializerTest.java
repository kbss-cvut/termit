...
class MultilingualStringSerializerTest {
    ...
    @Test
    void serializeWritesMultilingualStringAsMapOfLanguageToValuePairs() {
        ...
        value.getValue().forEach((lang, str) -> inOrder.verify(jsonGenerator).writeStringProperty(lang, str));
        inOrder.verify(jsonGenerator).writeEndObject();
    }

    @Test
    void serializeWritesNullLanguageAsEmptyKey() {
        final MultilingualString value = MultilingualString.create("test", "en");
        value.set("test");
        sut.serialize(value, jsonGenerator, null);
        verify(jsonGenerator).writeStringProperty("", value.get());
    }
}
